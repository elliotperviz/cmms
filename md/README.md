...
Molecular Dynamics (MD) is a computational simulation method that we will employ for materials modelling, where we consider matter to be made of a discrete arrangement of atoms. In MD, we study the time evolution of a system of atoms via the dynamical variation of the system state (i.e. positions and momenta of the atoms) by integrating Newton's Equations of motion. The time integration is implemented numerically, such that we solve to obtain the state of the system at discrete *timesteps* in a chosen time window. In practice, the timestep must be small enough to resolve the fastest atomic vibrations (typically on the order of femtoseconds), which limits the total simulated timescales to nanosceconds or microseconds. Typically, the number of atoms in the system can range from thousands to millions, far smaller than real materials. Thus, we understand both the power and limitation of MD simulations: it provides a detailed atomistic description of materials behaviour, but is a necessarily small and short-time view of materials behaviour.
...
Now, in fully *classical* MD we solve a coupled set of differential equations (N equations for N particles), using interatomic forces derived from a pre-determined classical *force-field* (FF). The FF is an analytical expression for the potential energy of the system, from which the forces follow by differentiation. In practice, the FF is always approximate: it is parameterised for a specific material or class of systems, and its functional form may be as simple as a two-body Lennard-Jones potential or as complex as a many-body reactive or machine-learning potential. Further, because evaluating interactions between all particle pairs scales as N<sup>2</sup>m efficient algorithms such as neighbour lists and cutoff distances are essential to make large-scale MD feasible. The complexity of the chosen force field, together with these scaling considerations, sets the practical limits on the size and duration of the simulations.
