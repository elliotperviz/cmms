...
If we wish to measure equilibrium properties, we **must** ensure the system is suitably equilibrated over the duration of the measurement. In the ideal case, Hamiltonian dynamics ensures that an isolated system conserves the total energy exactly, so in the microcanonical (NVE) ensemble the sum of kinetic and potential energies is constant. The system explores its accessible phase space, and no explicit “equilibration” would be needed if we somehow began from a representative ensemble of configurations. However, in practice, when we setup an MD simulation we face two important issues:
