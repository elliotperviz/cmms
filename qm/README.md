...
Before thinking about the numerical implementation, we will first simplify the problem: We introduce the Born-Oppenheimer (BO) approximation, which assumes that electrons react instantaneously to changes in nucleon position due to the large mass difference between a nucleon and an electron. In other words, we are able to follow a so-called "semi-classical" approach: classical "point-like" atoms and quantum electrons. Focusing on the quantum part, we set the kinetic energy of the nuclei to 0 by the definition of the BO approximation, and neglect (for now) the Coloumbic nucleus-nucleus interaction, which allows us to define a simplified *electronic* hamiltonian
```math
